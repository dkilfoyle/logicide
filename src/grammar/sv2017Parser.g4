...
unary_module_path_operator:
	...
	| NXOR
	| XORN;
unary_operator: PLUS | MINUS | unary_module_path_operator;
inc_or_dec_operator: INCR | DECR;
implicit_class_handle: KW_THIS ( DOT KW_SUPER)? | KW_SUPER;
integral_number:
	BASED_NUMBER_WITH_SIZE
	| ( UNSIGNED_NUMBER)? ANY_BASED_NUMBER
	| UNSIGNED_NUMBER;
real_number: REAL_NUMBER_WITH_EXP | FIXED_POINT_NUMBER;
...
charge_strength:
	LPAREN (KW_SMALL | KW_MEDIUM | KW_LARGE) RPAREN;

sequence_lvar_port_direction: KW_INPUT | KW_INOUT | KW_OUTPUT;
bins_keyword: KW_BINS | KW_ILLEGAL_BINS | KW_IGNORE_BINS;
class_item_qualifier: KW_STATIC | KW_PROTECTED | KW_LOCAL;
random_qualifier: KW_RAND | KW_RANDC;
property_qualifier: random_qualifier | class_item_qualifier;
method_qualifier: ( KW_PURE)? KW_VIRTUAL | class_item_qualifier;
...
any_implication: IMPLIES | IMPLIES_P | IMPLIES_N;
...
array_method_name:
	...
	| KW_XOR
	| identifier;
operator_mul_div_mod: MUL | DIV | MOD;
operator_plus_minus: PLUS | MINUS;
operator_shift:
	SHIFT_LEFT
	| SHIFT_RIGHT
	| ARITH_SHIFT_LEFT
	| ARITH_SHIFT_RIGHT;
operator_cmp: LT | LE | GT | GE;
operator_eq_neq:
	EQ
	| NE
	| CASE_EQ
	...
	| WILDCARD_EQ
	| WILDCARD_NE;
operator_xor: XOR | NXOR | XORN;
operator_impl: ARROW | BI_DIR_ARROW;
/****************************************** udp ***********************************************************************/
...
sequential_body: (udp_initial_statement)? KW_TABLE (
		sequential_entry
	)+ KW_ENDTABLE;
...
modport_item:
	identifier LPAREN modport_ports_declaration (
...
modport_simple_port: list_of_arguments_named_item | identifier;
modport_tf_ports_declaration:
	import_export modport_tf_port (COMMA modport_tf_port)*;
modport_tf_port: method_prototype | identifier;
/***************************************** statements  ****************************************************************/
statement_or_null: statement | ( attribute_instance)* SEMI;
...
event_control:
	AT (
		LPAREN ( MUL | event_expression) RPAREN
		| MUL
...
delay_value:
	...
	| ps_identifier;
delay_control:
	HASH (LPAREN mintypmax_expression RPAREN | delay_value);
nonblocking_assignment:
...
par_block:
	KW_FORK (COLON identifier | {_input->LA(1) != COLON}?) (
		block_item_declaration
	)* (statement_or_null)* join_keyword (
		COLON identifier
		| {_input->LA(1) != COLON}?
	);
/******************************************** case ********************************************************************/
...
case_item:
	(KW_DEFAULT ( COLON)? | expression ( COMMA expression)* COLON) statement_or_null;
case_pattern_item:
	(
		KW_DEFAULT ( COLON)?
...
case_inside_item:
	(KW_DEFAULT ( COLON)? | open_range_list COLON) statement_or_null;
...
disable_statement:
	KW_DISABLE (KW_FORK | hierarchical_identifier) SEMI;
event_trigger:
	(ARROW | DOUBLE_RIGHT_ARROW ( delay_or_event_control)?) hierarchical_identifier SEMI;
/*********************************************** loop statements ******************************************************/
loop_statement:
	(
		KW_FOREVER
		| (
			...
			| KW_FOR LPAREN (for_initialization)? SEMI (
				expression
			)? SEMI (for_step)?
		) RPAREN
	) statement_or_null
...
procedural_assertion_statement:
	...
	| checker_instantiation;
concurrent_assertion_statement:
	(KW_ASSERT | KW_ASSUME) KW_PROPERTY LPAREN property_spec RPAREN action_block
	| KW_COVER (
		KW_PROPERTY LPAREN property_spec
		| KW_SEQUENCE LPAREN (clocking_event)? (
			KW_DISABLE KW_IFF LPAREN expression_or_dist RPAREN
		)? sequence_expr
...
deferred_immediate_assert_statement:
	KW_ASSERT (KW_FINAL | primitive_delay) LPAREN expression RPAREN action_block;
deferred_immediate_assume_statement:
	KW_ASSUME (KW_FINAL | primitive_delay) LPAREN expression RPAREN action_block;
deferred_immediate_cover_statement:
	KW_COVER (KW_FINAL | primitive_delay) LPAREN expression RPAREN statement_or_null;
/**********************************************************************************************************************/
...
rs_case_item:
	(KW_DEFAULT ( COLON)? | expression ( COMMA expression)* COLON) production_item SEMI;
rs_rule:
	rs_production_list (
		DIST_WEIGHT_ASSIGN weight_specification (rs_code_block)?
	)?;
rs_production_list:
	KW_RAND KW_JOIN (LPAREN expression RPAREN)? production_item (
		production_item
	)+
	| ( rs_prod)+;
production:
	(data_type_or_void)? identifier (LPAREN tf_port_list RPAREN)? COLON rs_rule (
		BAR rs_rule
	)* SEMI;
tf_item_declaration:
	block_item_declaration
...
tf_port_item:
	(attribute_instance)* (tf_port_direction)? (KW_VAR)? (
		data_type_or_implicit
	)? (identifier ( variable_dimension)* ( ASSIGN expression)?)?;
tf_port_direction: KW_CONST KW_REF | port_direction;
tf_port_declaration:
	(attribute_instance)* tf_port_direction (KW_VAR)? (
		data_type_or_implicit
...
param_assignment:
	identifier (unpacked_dimension)* (
		ASSIGN constant_param_expression
	)?;
...
net_type_declaration:
	KW_NETTYPE (
		data_type identifier (KW_WITH package_or_class_scoped_id)?
	) SEMI;
...
let_formal_type: KW_UNTYPED | data_type_or_implicit;
/**********************************************************************************************************************/
package_import_declaration:
	KW_IMPORT package_import_item (COMMA package_import_item)* SEMI;
package_import_item: identifier DOUBLE_COLON ( MUL | identifier);
...
property_formal_type: KW_PROPERTY | sequence_formal_type;
...
property_instance:
	package_or_class_scoped_id (
		LPAREN property_list_of_arguments RPAREN
	)?;
property_spec:
	(clocking_event)? (
		KW_DISABLE KW_IFF LPAREN expression_or_dist RPAREN
...
property_expr:
	...
	| KW_IF LPAREN expression_or_dist RPAREN property_expr (
		KW_ELSE property_expr
		| {_input->LA(1) != KW_ELSE}?
	)
	| KW_CASE LPAREN expression_or_dist RPAREN (
		property_case_item
	)+ KW_ENDCASE
	| sequence_expr (
		(
	...
	| (
		KW_NOT
		| (KW_S_ALWAYS | KW_EVENTUALLY) LSQUARE_BR range_expression RSQUARE_BR
		| (
			KW_ACCEPT_ON
			| KW_REJECT_ON
			| KW_SYNC_ACCEPT_ON
...
package_or_class_scoped_id:
	(KW_DOLAR_UNIT | package_or_class_scoped_path_item) (
		DOUBLE_COLON package_or_class_scoped_path_item
	)*;
select:
	(DOT identifier | bit_select)* (
...
event_expression:
	event_expression_item (( KW_OR | COMMA) event_expression_item)*;
/************************************************ sequence_expr *******************************************************/
boolean_abbrev:
...
consecutive_repetition:
	LSQUARE_BR (MUL ( const_or_range_expression)? | PLUS) RSQUARE_BR;
non_consecutive_repetition:
...
sequence_instance:
	package_or_class_scoped_path (
		LPAREN (sequence_list_of_arguments)? RPAREN
	)?;
sequence_expr:
	KW_FIRST_MATCH LPAREN sequence_expr (
	...
	| sequence_expr (
		(KW_AND | KW_INTERSECT | KW_OR | KW_WITHIN) sequence_expr
		| ( cycle_delay_range sequence_expr)+
	)
	| (
...
sequence_match_item: operator_assignment | expression;
operator_assignment:
	variable_lvalue assignment_operator expression;
sequence_actual_arg: event_expression | sequence_expr;
dist_weight: ( DIST_WEIGHT_ASSIGN | COLON DIV) expression;
clocking_declaration:
	(
		KW_GLOBAL KW_CLOCKING (identifier)? clocking_event SEMI
		| (KW_DEFAULT)? KW_CLOCKING (identifier)? clocking_event SEMI (
			clocking_item
		)*
	) KW_ENDCLOCKING (
		COLON identifier
...
clocking_event:
	AT (identifier | LPAREN event_expression RPAREN);
cycle_delay_range:
	DOUBLE_HASH (
		LSQUARE_BR (
			MUL
...
expression_or_dist:
	expression (
		KW_DIST LBRACE dist_item (COMMA dist_item)* RBRACE
	)?;

covergroup_declaration:
	KW_COVERGROUP identifier (LPAREN tf_port_list RPAREN)? (
		coverage_event
	)? SEMI (coverage_spec_or_option)* KW_ENDGROUP (
		COLON identifier
		| {_input->LA(1) != COLON}?
	);
cover_cross:
	(identifier COLON)? KW_CROSS identifier_list_2plus (
		KW_IFF LPAREN expression RPAREN
	)? cross_body;
identifier_list_2plus: identifier ( COMMA identifier)+;
cross_body: LBRACE ( cross_body_item)* RBRACE | SEMI;
cross_body_item:
	function_declaration
...
bins_selection:
	bins_keyword identifier ASSIGN select_expression (
		KW_IFF LPAREN expression RPAREN
	)?;
select_expression:
	LPAREN select_expression RPAREN
	| ( NOT)? select_condition
	| select_expression (AND_LOG | OR_LOG) select_expression
...
coverage_spec_or_option:
	(attribute_instance)* (coverage_spec | coverage_option SEMI);
coverage_option:
	...
	| (
		(KW_WILDCARD)? bins_keyword identifier (
			(LSQUARE_BR ( covergroup_expression)? RSQUARE_BR)? ASSIGN (
				LBRACE covergroup_range_list RBRACE (
...
block_event_expression:
	(KW_BEGIN | KW_END) hierarchical_btf_identifier
...
variable_dimension:
	LSQUARE_BR (MUL | data_type | array_range_expression)? RSQUARE_BR;
...
enum_base_type:
	...
	| packed_dimension;
data_type_primitive: integer_type ( signing)? | non_integer_type;
data_type:
	KW_STRING
	| KW_CHANDLE
	...
	| (
		...
		| struct_union (KW_PACKED ( signing)?)? LBRACE (
			struct_union_member
		)+ RBRACE
		| package_or_class_scoped_path
	) (variable_dimension)*
...
primary_literal:
	...
	| KW_THIS
	| DOLAR;
type_reference: KW_TYPE LPAREN ( expression | data_type) RPAREN;
...
// original primary+constant_primary let_expression was remobed because it was same as call
primary:
	...
	| any_system_tf_identifier (
		LPAREN data_type (COMMA list_of_arguments)? (
	...
	| primary (DOT array_method_name)? (attribute_instance)* LPAREN (
		list_of_arguments
	)? RPAREN (KW_WITH LPAREN expression RPAREN)?												# PrimaryCall
	| primary DOT array_method_name																# PrimaryCallArrayMethodNoArgs
	| primary (DOT array_method_name)? (attribute_instance)* KW_WITH LPAREN expression RPAREN	#
		PrimaryCallWith;
/************************************************** expression ********************************************************/
...
expression:
	...
	| <assoc = right> expression operator_impl (
		attribute_instance
	)* expression;
...
variable_decl_assignment:
	identifier (
		...
		| (variable_dimension)+ (
			ASSIGN (expression | dynamic_array_new)
		)?
	)?;
...
stream_expression:
	expression (
		KW_WITH LSQUARE_BR array_range_expression RSQUARE_BR
	)?;
...
assignment_pattern:
	APOSTROPHE_LBRACE (
		...
		| constant_expression LBRACE expression (
			COMMA expression
		)* RBRACE
	)? RBRACE;
...
struct_union_member:
	(attribute_instance)* (random_qualifier)? data_type_or_void list_of_variable_decl_assignments
		SEMI;
data_type_or_void: KW_VOID | data_type;
enum_name_declaration:
	identifier (
		LSQUARE_BR integral_number (COLON integral_number)? RSQUARE_BR
	)? (ASSIGN expression)?;
...
constraint_expression:
	...
	| (
		KW_FOREACH LPAREN primary LSQUARE_BR loop_variables RSQUARE_BR RPAREN
		| expression ARROW
	) constraint_set;
uniqueness_constraint: KW_UNIQUE LBRACE open_range_list RBRACE;
constraint_set:
...
randomize_call:
	KW_RANDOMIZE (attribute_instance)* (
		...
		KW_WITH (LPAREN ( list_of_arguments)? RPAREN)? LBRACE (
			constraint_block_item
		)* RBRACE
	)?;
/**************************************** module *********************************************************************/
...
module_declaration:
	...
	)? SEMI (timeunits_declaration)? (module_item)* KW_ENDMODULE (
		COLON identifier
		| {_input->LA(1) != COLON}?
	);
...
net_port_type:
	...
	| data_type_or_implicit;
var_data_type: KW_VAR ( data_type_or_implicit)? | data_type;
...
list_of_variable_identifiers:
	list_of_variable_identifiers_item (
		COMMA list_of_variable_identifiers_item
	)*;
...
list_of_paths:
	identifier_doted_index_at_end (
		COMMA identifier_doted_index_at_end
	)*;
list_of_path_delay_expressions:
	...
	| t01_path_delay_expression COMMA t10_path_delay_expression COMMA t0z_path_delay_expression
		...
		COMMA t0x_path_delay_expression COMMA tx1_path_delay_expression COMMA
			t1x_path_delay_expression COMMA tx0_path_delay_expression COMMA
...
parallel_path_description:
	LPAREN specify_input_terminal_descriptor any_implication specify_output_terminal_descriptor
		RPAREN;
...
parallel_edge_sensitive_path_description:
	LPAREN (edge_identifier)? specify_input_terminal_descriptor any_implication LPAREN
		specify_output_terminal_descriptor (operator_plus_minus)? COLON data_source_expression
		RPAREN RPAREN;
full_edge_sensitive_path_description:
	LPAREN (edge_identifier)? list_of_path_inputs (
		operator_plus_minus
	)? PATH_FULL LPAREN list_of_path_outputs (
		operator_plus_minus
	)? COLON data_source_expression RPAREN RPAREN;
data_source_expression: expression;
data_declaration:
	(KW_CONST)? (
		KW_VAR ( lifetime)? ( data_type_or_implicit)?
...
state_dependent_path_declaration:
	KW_IF LPAREN module_path_expression RPAREN (
		simple_path_declaration
		| edge_sensitive_path_declaration
	)
...
genvar_declaration: KW_GENVAR identifier_list SEMI;
net_declaration:
	(
		KW_INTERCONNECT (implicit_data_type)? (HASH delay_value)? identifier (
			unpacked_dimension
...
list_of_port_declarations:
	LPAREN (
		...
		| (
			list_of_port_declarations_ansi_item (
				COMMA list_of_port_declarations_ansi_item
			)*
		)
	)? RPAREN;
...
nonansi_port__expr:
	identifier_doted_index_at_end
	| LBRACE identifier_doted_index_at_end (
		COMMA identifier_doted_index_at_end
	)* RBRACE;
...
dolar_setup_timing_check:
	KW_DOLAR_SETUP LPAREN timing_check_event COMMA timing_check_event COMMA timing_check_limit (
		COMMA ( notifier)?
	)? RPAREN SEMI;
dolar_hold_timing_check:
	KW_DOLAR_HOLD LPAREN timing_check_event COMMA timing_check_event COMMA timing_check_limit (
		COMMA ( notifier)?
	)? RPAREN SEMI;
dolar_setuphold_timing_check:
	KW_DOLAR_SETUPHOLD LPAREN timing_check_event COMMA timing_check_event COMMA timing_check_limit
		...
		COMMA (notifier)? (
			COMMA (timestamp_condition)? (
				COMMA (timecheck_condition)? (
...
dolar_removal_timing_check:
	KW_DOLAR_REMOVAL LPAREN timing_check_event COMMA timing_check_event COMMA timing_check_limit (
		COMMA ( notifier)?
	)? RPAREN SEMI;
dolar_recrem_timing_check:
	KW_DOLAR_RECREM LPAREN timing_check_event COMMA timing_check_event COMMA timing_check_limit
		COMMA timing_check_limit (
		COMMA (notifier)? (
			COMMA (timestamp_condition)? (
				COMMA (timecheck_condition)? (
...
dolar_skew_timing_check:
	KW_DOLAR_SKEW LPAREN timing_check_event COMMA timing_check_event COMMA timing_check_limit (
		COMMA ( notifier)?
	)? RPAREN SEMI;
dolar_timeskew_timing_check:
...
dolar_width_timing_check:
	KW_DOLAR_WIDTH LPAREN controlled_reference_event COMMA timing_check_limit COMMA threshold (
		COMMA ( notifier)?
	)? RPAREN SEMI;
dolar_nochange_timing_check:
	KW_DOLAR_NOCHANGE LPAREN timing_check_event COMMA timing_check_event COMMA start_edge_offset
		COMMA end_edge_offset (COMMA ( notifier)?)? RPAREN SEMI;
...
function_data_type_or_implicit:
	...
	| implicit_data_type;
extern_tf_declaration:
	KW_EXTERN (KW_FORKJOIN task_prototype | method_prototype) SEMI;
function_declaration:
	KW_FUNCTION (lifetime)? (function_data_type_or_implicit)? task_and_function_declaration_common
		KW_ENDFUNCTION (
		COLON identifier
		| {_input->LA(1) != COLON}?
	);
...
function_prototype:
	KW_FUNCTION data_type_or_void identifier (
...
task_declaration:
	KW_TASK (lifetime)? task_and_function_declaration_common KW_ENDTASK (
		COLON identifier
		| {_input->LA(1) != COLON}?
	);
...
checker_port_item:
	(attribute_instance)* (checker_port_direction)? (
		property_formal_type
	)? identifier (variable_dimension)* (
		ASSIGN property_actual_arg
	)?;
checker_port_direction: KW_INPUT | KW_OUTPUT;
checker_declaration:
...
class_declaration:
	(KW_VIRTUAL)? KW_CLASS (lifetime)? identifier (
		...
		KW_EXTENDS class_type (
			LPAREN ( list_of_arguments)? RPAREN
		)?
	)? (
		KW_IMPLEMENTS interface_class_type (
			COMMA interface_class_type
		)*
	)? SEMI (class_item)* KW_ENDCLASS (
		COLON identifier
		| {_input->LA(1) != COLON}?
	);
...
interface_class_declaration:
	KW_INTERFACE KW_CLASS identifier (parameter_port_list)? (
		KW_EXTENDS interface_class_type (
			COMMA interface_class_type
		)*
	)? SEMI (interface_class_item)* KW_ENDCLASS (
		COLON identifier
		| {_input->LA(1) != COLON}?
	);
...
package_declaration:
	(attribute_instance)* KW_PACKAGE (lifetime)? identifier SEMI (
		timeunits_declaration
	)? (( attribute_instance)* package_item)* KW_ENDPACKAGE (
		COLON identifier
		| {_input->LA(1) != COLON}?
	);
package_item:
...
sequence_declaration:
	...
	)* sequence_expr (SEMI)? KW_ENDSEQUENCE (
		COLON identifier
		| {_input->LA(1) != COLON}?
	);
sequence_port_list:
...
sequence_port_item:
	(attribute_instance)* (
		KW_LOCAL ( sequence_lvar_port_direction)?
	)? (sequence_formal_type)? identifier (variable_dimension)* (
		ASSIGN sequence_actual_arg
	)?;
property_declaration:
	KW_PROPERTY identifier (LPAREN ( property_port_list)? RPAREN)? SEMI (
		assertion_variable_declaration
	)* property_spec (SEMI)? KW_ENDPROPERTY (
		COLON identifier
		| {_input->LA(1) != COLON}?
	);
property_port_list:
...
property_port_item:
	(attribute_instance)* (KW_LOCAL ( KW_INPUT)?)? (
		property_formal_type
	)? identifier (variable_dimension)* (
		ASSIGN property_actual_arg
	)?;
...
continuous_assign:
	KW_ASSIGN (
		(drive_strength)? (delay3)? list_of_variable_assignments
...
class_constructor_declaration:
	KW_FUNCTION (class_scope)? KW_NEW (
		LPAREN tf_port_list RPAREN
	)? SEMI (block_item_declaration)* (
		KW_SUPER DOT KW_NEW (LPAREN ( list_of_arguments)? RPAREN)? SEMI
	)? (statement_or_null)* KW_ENDFUNCTION (COLON KW_NEW)?;
class_property:
	KW_CONST (class_item_qualifier)* data_type identifier (
		ASSIGN constant_expression
	)? SEMI
	| ( property_qualifier)* data_declaration;
class_method:
	KW_PURE KW_VIRTUAL (class_item_qualifier)* method_prototype SEMI
	| KW_EXTERN (method_qualifier)* (
...
gate_instantiation:
	(
		...
		| (cmos_switchtype | mos_switchtype) (delay3)?
			enable_gate_or_mos_switch_or_cmos_switch_instance (
			COMMA enable_gate_or_mos_switch_or_cmos_switch_instance
		)*
...
enable_gate_or_mos_switch_or_cmos_switch_instance:
	(name_of_instance)? LPAREN output_terminal COMMA input_terminal COMMA expression (
		COMMA expression
	)? RPAREN;
n_input_gate_instance:
	(name_of_instance)? LPAREN output_terminal (
		COMMA input_terminal
	)+ RPAREN;
n_output_gate_instance:
	(name_of_instance)? LPAREN output_terminal (
		COMMA output_terminal
	)* COMMA input_terminal RPAREN;
...
udp_instantiation:
	identifier (
		drive_strength (delay2)? (name_of_instance)?
		| delay2 ( name_of_instance)?
	)? udp_instance_body (COMMA udp_instance)* SEMI;
udp_instance: ( name_of_instance)? udp_instance_body;
udp_instance_body:
...
bind_directive:
	KW_BIND (
		identifier (COLON bind_target_instance_list)?
...
config_declaration:
	...
	)* KW_ENDCONFIG (COLON identifier | {_input->LA(1) != COLON}?);
design_statement:
	KW_DESIGN (( identifier DOT)? identifier)* SEMI;
config_rule_statement:
	(
		KW_DEFAULT liblist_clause
		| (inst_clause | cell_clause) (
			liblist_clause
			| use_clause
		)
	) SEMI;
...
if_generate_construct:
	KW_IF LPAREN constant_expression RPAREN generate_item (
		KW_ELSE generate_item
		| {_input->LA(1) != KW_ELSE}?
	);
case_generate_construct:
	KW_CASE LPAREN constant_expression RPAREN (
		case_generate_item
	)+ KW_ENDCASE;
case_generate_item:
	(
...
elaboration_system_task:
	(
		KW_DOLAR_FATAL (
			LPAREN UNSIGNED_NUMBER (COMMA ( list_of_arguments)?)? RPAREN
		)?
