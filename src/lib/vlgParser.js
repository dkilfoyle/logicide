...
const singleLineComment = sequenceOf([str("//"), everythingUntil(char("\n")), optionalWhitespace]);

const multipleLineComment = sequenceOf([str("/*"), everythingUntil(str("*/")), str("*/")]);

const optionalWhitespaceOrComment = many(choice([whitespace, singleLineComment, multipleLineComment])).map((x) => x || "");

const ws = (myparser) => between(optionalWhitespaceOrComment)(optionalWhitespaceOrComment)(myparser);
...
const keywords = ["module", "begin", "input", "output", "wire", "assign", "endmodule", "buffer", "and", "not", "or", "xor", "nor", "nand"];

const identifier = coroutine(function*() {
  const id = yield either(
    ws(sequenceOf([regex(/^[a-zA-Z_]/), possibly(regex(/^[a-zA-Z0-9_]+/)).map((x) => (x === null ? "" : x))])).map((x) => x.join(""))
  );
  if (id.isError) {
    yield fail("Invalid ID");
  }
  if (keywords.some((x) => x == id.value)) yield fail("Identifier cannot be keyword");
  return id.value;
});
...
const definedVariableParser = coroutine(function*() {
  ...
  if (!definedVariables.some((x) => x == variable)) {
    yield tapParser((state) => lintError("Undefined variable", "warning")("", state.index - variable.length));
  }
  return variable;
});
...
const definedControlParser = coroutine(function*() {
  ...
  if (!definedControls.some((x) => x == variable)) {
    yield tapParser((state) => lintError("Must be control variable", "warning")("", state.index - variable.length));
  }
  return variable;
});
...
const instanceParser = coroutine(function*() {
  ...
  const module = yield ws(identifier).errorMap(() => "no module");
  const id = yield ws(identifier).errorMap(() => "no id");
  const parameters = yield betweenRoundBrackets(commaSeparated(ws(instanceParamsParser)));
  yield str(";");
...
const gateParser = coroutine(function*() {
  ...
  const params = yield betweenRoundBrackets(commaSeparated(definedVariableParser));

  if (["control", "buffer", "response"].includes(gate) && params.length != 1) {
    yield lintWarningParser(`'${gate}' expects exactly 1 parameter`, "warning", {
      from: start,
    });
  }

  if (["and", "nand", "or", "xor", "nor"].includes(gate) && params.length < 3) {
    yield lintWarningParser(`'${gate}' expects at least 3 parameters`, "warning", {
      from: start,
    });
  }
...
const operator = choice([
  char("&").map(() => ({ type: "OP_AND", value: "and" })),
  char("|").map(() => ({ type: "OP_OR", value: "or" })),
  char("^").map(() => ({ type: "OP_XOR", value: "xor" })),
  str("~&").map(() => ({ type: "OP_NAND", value: "nand" })),
  str("~|").map(() => ({ type: "OP_NOR", value: "nor" })),
]);
const variable = sequenceOf([possibly(char("~")), definedVariableParser]).map(([n, v]) => ({ type: "VARIABLE", value: v, invert: n != null }));
...
const assignParser = coroutine(function*() {
  yield str("assign ");
  const id = yield ws(identifier).errorMap(lintError("Invalid assign identifier"));
  yield ws(str("="));
  const value = yield squareBracketExpr.errorMap(lintError("Invalid expression"));
...
const arrayDimParser = coroutine(function*() {
  const dim = yield betweenSquareBrackets(sequenceOf([digits, str(":"), digits])).map((x) => [x[0], x[2]]);
  return dim;
});

const listOfIds = sequenceOf([ws(identifier), many(takeRight(str(","))(ws(identifier)))])
  .map((x) => x.flat())
  .errorMap(lintError("Invalid port identifier"));

const portParser = coroutine(function*() {
  const type = yield choice([ws(str("input")), ws(str("output"))]).errorMap(lintError("Invalid port type: must be 'input' or 'output'"));
...
const wireParser = coroutine(function*() {
  yield str("wire ");
  const vars = yield commaSeparated(ws(identifier).errorMap(lintError("Invalid wire identifier")));
  yield ws(str(";"));
  return vars;
});
...
const moduleParser = coroutine(function*() {
  ...
  const id = yield ws(identifier).errorMap(lintError("Invalid module identifier"));

  const ports = yield possibly(
    betweenRoundBrackets(commaSeparated(portParser.errorMap(lintError("Invalid ports section")))).map((x) => x.flat())
  ).map((x) => x || []);
  // ports = ports || [];
  ...
  var clock;
  if (id == "main") clock = yield testParser.errorMap(lintError("Invalid test block"));
...
const testAssignmentParser = coroutine(function*() {
  ...
  yield str("=");
  const value = yield sequenceOf([ws(binaryDigit), lookAhead(anythingExcept(binaryDigit))])
    .map((x) => x[0])
...
const testTimeParser = coroutine(function*() {
  ...
  yield optionalWhitespace;
  var assignments = yield possibly(betweenCurlyBrackets(commaSeparated(ws(testAssignmentParser))));
